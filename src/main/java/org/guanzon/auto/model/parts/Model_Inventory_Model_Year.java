 ...
 * @author Arsiela
 */
public class Model_Inventory_Model_Year implements GEntity {
     ...
     */
    public Model_Inventory_Model_Year(GRider foValue) {
        if (foValue == null) {
     ...
     * @param fsValue1 - filter values
     * @param fnValue2 - filter values2
     * @return result as success/failed
     */
    public JSONObject openRecord(String fsValue, String fsValue1, Integer fnValue2) {
        poJSON = new JSONObject();
        ...
        lsSQL = MiscUtil.addCondition(lsSQL, " a.sStockIDx = " + SQLUtil.toSQL(fsValue)
                                               + " AND a.sModelCde = " + SQLUtil.toSQL(fsValue1)
                                               + " AND a.nYearModl = " + SQLUtil.toSQL(fnValue2));
    ...
    public JSONObject saveRecord() {
        ...
        if (pnEditMode == EditMode.ADDNEW || pnEditMode == EditMode.UPDATE) {
            ...
            if (pnEditMode == EditMode.ADDNEW) {
                ...
                Model_Inventory_Model_Year loOldEntity = new Model_Inventory_Model_Year(poGRider);

                //replace with the primary key column info
                JSONObject loJSON = loOldEntity.openRecord(this.getStockID(),this.getModelCde(), this.getYearModl());

                if ("success".equals((String) loJSON.get("result"))) {
                    //replace the condition based on the primary key column of the record
                    lsSQL = MiscUtil.makeSQL(this, loOldEntity, " sStockIDx = " + SQLUtil.toSQL(this.getStockID()) + " AND sModelCde = " + SQLUtil.toSQL(this.getModelCde()) + "AND nYearModl = " + SQLUtil.toSQL(this.getYearModl()),lsExclude);
    ...
    public JSONObject deleteRecord(){
        ...
        String lsSQL = " DELETE FROM "+getTable()+" WHERE "
                    ...
                    + " AND nYearModl = " + SQLUtil.toSQL(this.getYearModl());
        if (!lsSQL.isEmpty()) {
            if (poGRider.executeQuery(lsSQL, getTable(), poGRider.getBranchCode(), "") > 0) {
                poJSON.put("result", "success");
